...
#   - 도상은 말하지 않는다(SPIRIT §2). 미륵은 제 일을 한다 — 솟는다. 다만
#     사용자를 부르지 않고, 다 올라와도 웃거나 빛나 조르지 않는다.
class Maitreya
  FULL = 24

  # 비운 날 수 → 그림이 땅 위로 드러난 비율. 초반은 빠르고 후반은 천천히
  # (ease-out) — 첫 달에 흙 둔덕만 보면 답답하다. 사이는 곧게 잇는다.
  #   0 갓 끝만 · 6 갓과 이마 · 12 얼굴이 다 나온다(턱까지) · 24 가슴까지
  STAGES = { 0 => 0.055, 6 => 0.30, 12 => 0.50, 24 => 0.70 }.freeze
